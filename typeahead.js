        ...
        getSourceObject: function (value, func, attr) { // default API oriented function
            if (horecaTechTypeAhead.data[attr.typeaheadid].URL != "") {
                function request(value, params, func, attr) {
                    $.getJSON(horecaTechTypeAhead.data[attr.typeaheadid].URL + (horecaTechTypeAhead.data[attr.typeaheadid].JSONP ? '?callback=?' : ''),
                            params,
                            function (data) {
...
// наложение импортируемых на шаблон 
        ...
        this.on("click.httypeahead keyup.httypeahead", function (e) {
            function get(targetID) {
                var value = $('[typeaheadid="' + targetID + '"]').val();
            ...
            function build(value, valueID, targetID) {
                function sort_func(a, b) {
                    if (a[horecaTechTypeAhead.data[targetID].SourceSortKey_] < b[horecaTechTypeAhead.data[targetID].SourceSortKey_])
                        return -1;
                    if (a[horecaTechTypeAhead.data[targetID].SourceSortKey_] > b[horecaTechTypeAhead.data[targetID].SourceSortKey_])
                        return 1;
                    return 0;
                ...
                ;
                if (valueID == null)
                    valueID = horecaTechTypeAhead.data[targetID].prevValueID;
                if (horecaTechTypeAhead.data[targetID].Source[valueID] != null && (horecaTechTypeAhead.data[targetID].prevValue != value || horecaTechTypeAhead.data[targetID].Source[valueID].built == null)) {
                    ...
                    if (horecaTechTypeAhead.data[targetID].SourceType_ == null)
                        horecaTechTypeAhead.data[targetID].SourceType_ = (horecaTechTypeAhead.data[targetID].Source[valueID].Array.prototype.toString.call(arguments[2]) == "[object Array]") ? "A" : "O";
                    ...
                    if (horecaTechTypeAhead.data[targetID].Array.length == 0) {
                        ...
                        var content_height = $("#httypeahead_container_" + targetID).height(),
                                content_height_to_show = Math.min(horecaTechTypeAhead.data[targetID].MaxLines * horecaTechTypeAhead.data[targetID].rowHeight, content_height),
                        ...
                        $(".httypeahead_element_class_" + targetID).on("mouseout.httypeahead", function (e) {
                            $("." + $(this).attr("class")).find('span').css({color: horecaTechTypeAhead.data[targetID].AccentuationColor})
                        ...
                        $(".httypeahead_element_class_" + targetID).on("mouseover.httypeahead", function (e) {
                            $(this).find('span').css({color: horecaTechTypeAhead.data[targetID].SelectedAccentuationColor})
            ...
            if (e.type=='click' && $("#httypeahead_frame_" + targetID).length == 0) {
                ...
                var css = {}
                for (var cssAttr in  horecaTechTypeAhead.cssAttrArray) {
                    if (cssAttr.indexOf("padding") == -1) {
                        css[cssAttr] = horecaTechTypeAhead.data[targetID][cssAttr]
                    }
                }
                var frame = $("#httypeahead_frame_" + targetID)
                frame.css(css).css({
                    borderTopLeftRadius: horecaTechTypeAhead.data[targetID].borderRadius.Bottom.Left,
                    borderTopRightRadius: horecaTechTypeAhead.data[targetID].borderRadius.Bottom.Right,
                    borderBottomLeftRadius: horecaTechTypeAhead.data[targetID].borderRadius.Top.Left,
                    borderBottomRightRadius: horecaTechTypeAhead.data[targetID].borderRadius.Top.Right
                });
                ...
                horecaTechTypeAhead.data[targetID].rowHeight = $(horecaTechTypeAhead.data[targetID].target).height() + parseInt($(horecaTechTypeAhead.data[targetID].target).css("padding-top")) + parseInt($(horecaTechTypeAhead.data[targetID].target).css("padding-bottom")) + ((horecaTechTypeAhead.data[targetID].LinesSeparator) ? 1 : 0);

                var stylehtml = ".httypeahead_element_class_" + targetID + " {padding-right:" + horecaTechTypeAhead.data[targetID].paddingRight + ";padding-top:" + horecaTechTypeAhead.data[targetID].paddingTop + ";padding-bottom:" + horecaTechTypeAhead.data[targetID].paddingBottom + ";padding-left:" + horecaTechTypeAhead.data[targetID].paddingLeft + ";max-width:" + $(window).width() + ";margin:0;color:" + horecaTechTypeAhead.data[targetID].color + ";width:auto;overflow:hidden;position:relative;display:block;" + ((horecaTechTypeAhead.data[targetID].LinesSeparator) ? ("border-top:1px solid " + horecaTechTypeAhead.data[targetID].LinesSeparatorColor) : "") + "}";
